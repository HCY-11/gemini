...
void Swapchain::create(const Window& window, const Surface& surface, const GPU& gpu, const Device& device)
{
    ...
    CHECK_VK_RESULT(vkCreateSwapchainKHR(device.get(), &swapchainInfo, nullptr, &m_swapchain), "Failed to create swapchain!");
...
VkExtent2D Swapchain::chooseExtent(const Window& window)
{
...
void Swapchain::createImageViews(const Device& device)
{
    ...
    for (uint32_t i = 0; i < m_imageViews.size(); i++)
    {
        ...
        CHECK_VK_RESULT(vkCreateImageView(device.get(), &viewInfo, nullptr, &m_imageViews[i]),
