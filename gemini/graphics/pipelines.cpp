...
void PipelineBuilder::populateStateInfosDefault(PipelineInfo& info, const Swapchain& swapchain)
{
...
void PipelineBuilder::build(const Device& device, const RenderPass& renderPass, Pipeline& dst)
{
    ...
    CHECK_VK_RESULT(vkCreatePipelineLayout(device.get(), &layoutInfo, nullptr, &dst.layout), "Failed to create pipeline layout!");
    ...
    CHECK_VK_RESULT(vkCreateGraphicsPipelines(device.get(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &dst.value),
...
VkShaderModule PipelineBuilder::createShaderModule(const Device& device, const char* filePath)
{
    std::ifstream file(filePath, std::ios::ate | std::ios::binary);

    if (!file.is_open())
    {
        LOG("Failed to find shader file " + std::string(filePath));
    }
    ...
    CHECK_VK_RESULT(vkCreateShaderModule(device.get(), &moduleInfo, nullptr, &shaderModule), 
